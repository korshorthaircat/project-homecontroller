...
const NavProductCategoryList = () => {
  ...
  useEffect(() => {
    ...
    if (params) {
      const tempObj = categoryList.filter(
        (category) => category.title === params.productCategoryName
      );

      console.log(tempObj);
      setParamCategory((prev) => tempObj[0]);
    }
  }, [params]);
  ...
  useEffect(() => {
    async function fetchData() {
      const result = await axios.get(
        "http://localhost:8080/api/product/getProductCategoryList?productCode=" +
          paramCategory.code
      );
    ...
    if (Object.keys(paramCategory).length !== 0) {
      fetchData();
    }
  }, [paramCategory]);
  ...
  return (
    <div className="cute">
      ...
      {Object.keys(paramCategory).length !== 0 && (
        <Categoryinfo item={paramCategory} />
      )}
      <Box sx={{ flexGrow: 1 }}>
